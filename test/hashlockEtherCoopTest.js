...
contract('Test Hashlocked Ether Payments', function(accounts) {
  ...
  it("Alice generates htl ether channel payment", async () => {
    ...
    htltx1.push(1) // amount
    htltx1.push(web3.sha3(tx_secret)) // hash lock
    htltx1.push(0) // timeout length
  ...
  it("generate close channel state", async () => {
    ...
    inputs.push(metachannelCTFaddress) // counterfactual metachannel address
    inputs.push('0x0') // sub-channel root hash
    inputs.push(web3.toWei(9, 'ether')) // balance in ether partyA
    inputs.push(web3.toWei(21, 'ether')) // balance in ether partyB
